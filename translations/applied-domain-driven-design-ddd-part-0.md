...
大约在一年前 (本文作者写于 2014 年 12 月), 我写过一系列有关领域驱动设计的文章 (这个系列将陆续翻译出), 你可以在这里找到它的[开篇章节](http://www.zankavtaskin.com/2013/09/applied-domain-driven-design-ddd-part-1.html). 回顾过去, 我意识到自己犯了最典型的错误, 最开始, 在没有任何的需求设计和草稿设计的情况下, 就对我的业务领域开始写代码了.

## 为此, 我打算尝试修复这个问题, 以下是我准备要做的 (I am going to try and fix this, this is what I am going to do)
...
- 组合 *仅展示有趣* 的交互 UML 类图 (Put together class digram UML *showing only interesting interactions*)

## 这是我编造的一些用户故事 (So here are made up user stories)
...
- 作为客户, 我希望能够在结账的时候指定我的信用卡资料, 以便于我能够支付商品.
...
现在, 我要开始提取上述故事的名字和动词了. 我在寻找那些将成为名词的主要对象, 而不是属性.
...
*注意: 为了更好, 我把重复的更正式的名字已经删除了, 比如 商品 (Item) = 产品 (Product), 订单 (Order) = 采购 (Purchase)
...
- 查看在自己国家的总税额. (See total tax for my country)
- 设定交付地址. (Specify delivery address)
- 为交付地址添加交付小记. (Specify delivery note for delivery address)
- 这顶发票地址. (Specify invoice address)
- 接收订单的发票. (Receive invoice for the order)
- 发送发票. (Sent invoice)
- 设定信用卡资料. (Specify credit card information)
- 为商品付款. (Pay for the items)
- 告诉我商品有多少存货. (Tell me how many items are in stock)
- 检查在结账的时候还剩余多少商品. (Check that items are still available during check out)
- 接收订单的确认邮件. (Receive order confirmation email)d

> By using above nouns and verbs we can put together a diagram such as this:

通过使用上面的名词和动词, 我们能够组合出像这样一个关系图:
...
一旦我们有了对象交互关系图, 我们就能够开始关于对象职责的思考了. 最常见的错误之一是将职责推给角色对象, 比如 `客户` (Customer). 我们应该牢记, 对象必须自己照顾自己, 对象需要对直接通信进行封闭, 而是通过函数与他们通信.
...
图三展示了方法, 类名, 依赖项, 接口和组合. 我花了一些时间, 只考虑了模型中最复杂最有趣的部分. 稍后我将考虑属性 (特性) 和其它细节. 当我开始编码时, 细节会自然而然地浮现. 假设图三是个粗略的草图, 也就是说, 团队可以在会议期间, 在白板上画出图三, 拍照并分发给团队中的每个人, 然后开始实际编码. 大约一周后, 图片就会被遗忘, 并且上面模型的部分 (有用的部分) 就会在实际的代码中起到作用.
